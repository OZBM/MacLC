...
struct pl_color_space vlc_placebo_ColorSpace(const video_format_t *fmt)
{
    ...
    float prim_scale = 1.0f / 50000.0f;
    float luma_scale = 1.0f / 10000.0f;
    ...
    return (struct pl_color_space) {
        ...
        .hdr = {
            .prim = {
                .green.x = fmt->mastering.primaries[0] * prim_scale,
                .green.y = fmt->mastering.primaries[1] * prim_scale,
                .blue.x  = fmt->mastering.primaries[2] * prim_scale,
                .blue.y  = fmt->mastering.primaries[3] * prim_scale,
                .red.x   = fmt->mastering.primaries[4] * prim_scale,
                .red.y   = fmt->mastering.primaries[5] * prim_scale,
                .white.x = fmt->mastering.white_point[0] * prim_scale,
                .white.y = fmt->mastering.white_point[1] * prim_scale,
            },
            .min_luma = fmt->mastering.min_luminance * luma_scale,
            .max_luma = (fmt->mastering.max_luminance < 10000 && fmt->mastering.max_luminance > 0)
                        ? (float)fmt->mastering.max_luminance
                        : fmt->mastering.max_luminance * luma_scale,
