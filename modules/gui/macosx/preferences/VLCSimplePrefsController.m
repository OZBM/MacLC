...
static inline char *safe_config_GetPsz(const char *name)
{
    if (!name) return NULL;
    module_config_t *p_item = config_FindConfig(name);
    if (!p_item || !IsConfigStringType(p_item->i_type))
        return NULL;
    return (config_GetPsz)(name);
}

static inline int64_t safe_config_GetInt(const char *name)
{
    if (!name) return 0;
    module_config_t *p_item = config_FindConfig(name);
    if (!p_item || !IsConfigIntegerType(p_item->i_type))
        return 0;
    return (config_GetInt)(name);
}

static inline float safe_config_GetFloat(const char *name)
{
    if (!name) return 0.0f;
    module_config_t *p_item = config_FindConfig(name);
    if (!p_item || !IsConfigFloatType(p_item->i_type))
        return 0.0f;
    return (config_GetFloat)(name);
}

static inline const char *safe_config_GetLabel(const char *name)
{
    if (!name) return NULL;
    module_config_t *p_item = config_FindConfig(name);
    if (!p_item)
        return NULL;
    return (config_GetLabel)(name);
}

static inline void safe_config_PutPsz(const char *name, const char *value)
{
...
static inline void safe_config_PutInt(const char *name, int64_t value)
{
    if (!name) return;
    module_config_t *p_item = config_FindConfig(name);
    if (!p_item || !IsConfigIntegerType(p_item->i_type))
...
static inline void safe_config_PutFloat(const char *name, float value)
{
    if (!name) return;
    module_config_t *p_item = config_FindConfig(name);
    if (!p_item || !IsConfigFloatType(p_item->i_type))
...
#define config_GetPsz(x) safe_config_GetPsz(x)
#define config_GetInt(x) safe_config_GetInt(x)
#define config_GetFloat(x) safe_config_GetFloat(x)
#define config_GetLabel(x) safe_config_GetLabel(x)
#define config_PutPsz(x, y) safe_config_PutPsz(x, y)
#define config_PutInt(x, y) safe_config_PutInt(x, y)
#define config_PutFloat(x, y) safe_config_PutFloat(x, y)
...
- (void)initStrings
{
    ...
    [_intf_statusIconCheckbox setTitle: _NS("Display MacLC status menu icon")];
    ...
    /* HDR / EDR */
    [_video_hdrBox setTitle: _NS("High Dynamic Range (HDR) & EDR")];
    [_video_hdrModeLabel setStringValue: _NS("HDR output mode")];
...
- (void)setupButton: (NSButton *)object forBoolValue: (const char *)name
{
    ...
    const char *label = config_GetLabel(name);
    if (label)
        [object setToolTip: NSTR(label)];
}

- (void)setupField:(NSTextField *)object forOption:(const char *)psz_option
{
    if (!object || !psz_option)
        return;
    module_config_t *p_item = config_FindConfig(psz_option);
    if (!p_item || !IsConfigStringType(p_item->i_type))
        return;
    ...
    const char *label = config_GetLabel(psz_option);
    if (label)
        [object setToolTip: NSTR(label)];
}
...
- (BOOL)hasModule:(NSString *)moduleName inConfig:(NSString *)config
{
    if (!config)
        return NO;
    const char *psz_config = [config UTF8String];
    module_config_t *p_item = config_FindConfig(psz_config);
    if (!p_item || !IsConfigStringType(p_item->i_type))
        return NO;
    char *value = config_GetPsz(psz_config);
...
- (void)changeModule:(NSString *)moduleName inConfig:(NSString *)config enable:(BOOL)enable
{
    ...
    config_PutPsz(psz_config, [[components componentsJoinedByString:@":"] UTF8String]);
}
...
- (void)resetControls
{
    ...
    /* HDR / EDR settings */
    if (_video_hdrModePopup) {
        [_video_hdrModePopup removeAllItems];
        [_video_hdrModePopup addItemWithTitle: _NS("Auto (Native EDR on HDR screens)")];
        [[_video_hdrModePopup lastItem] setTag: 0];
        [_video_hdrModePopup addItemWithTitle: _NS("Force Native EDR / HDR")];
        [[_video_hdrModePopup lastItem] setTag: 1];
        [_video_hdrModePopup addItemWithTitle: _NS("Tone-map to SDR")];
        [[_video_hdrModePopup lastItem] setTag: 2];
        [_video_hdrModePopup addItemWithTitle: _NS("Disable HDR")];
        [[_video_hdrModePopup lastItem] setTag: 3];
        [_video_hdrModePopup selectItemWithTag: config_GetInt("macosx-hdr-mode")];
    }
    if (_video_edrHeadroomTextField) {
        [_video_edrHeadroomTextField setStringValue: [NSString stringWithFormat:@"%.1f", config_GetFloat("macosx-edr-headroom")]];
    }
    if (_video_hdrStatusLabel) {
        if (@available(macOS 10.15, *)) {
            NSScreen *screen = [NSScreen mainScreen];
            CGFloat maxEDR = [screen maximumExtendedDynamicRangeColorComponentValue];
            if (maxEDR > 1.0) {
                [_video_hdrStatusLabel setStringValue: [NSString stringWithFormat:_NS("Display supports HDR (Peak EDR: %.1fx)"), maxEDR]];
            } else {
                [_video_hdrStatusLabel setStringValue: _NS("Display HDR: SDR mode (Peak EDR: 1.0x)")];
            }
        } else {
            [_video_hdrStatusLabel setStringValue: _NS("HDR requires macOS 10.15 or later")];
        }
    }
...
- (IBAction)resetPreferences:(NSControl *)sender
{
    ...
    [alert setInformativeText:_NS("This will reset MacLC's preferences.\n\n"
                                  "Note that MacLC will restart during the process, so your current "
...
#define SaveModuleList(object, name) SaveStringList(object, name)
    ...
    if (_videoSettingChanged) {
        ...
        if (_video_hdrModePopup)
            SaveIntList(_video_hdrModePopup, "macosx-hdr-mode");
        if (_video_edrHeadroomTextField)
            config_PutFloat("macosx-edr-headroom", [_video_edrHeadroomTextField floatValue]);
