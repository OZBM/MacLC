 ...
 * kCVImageBufferMasteringDisplayColorVolumeKey, kCVImageBufferContentLightLevelInfoKey
 * and kCVImageBufferGammaLevelKey if correspondence is found from a video_format_t.
...
/**
 * @brief Create CFDataRef with SMPTE ST 2086 mastering display color volume
 * 
 * @param fmt The video format with mastering info
 * @return CFDataRef or NULL
 */
CFDataRef cvpx_create_mastering_display_color_volume_data(const video_format_t *fmt);

/**
 * @brief Create CFDataRef with CTA-861.3 content light level info
 * 
 * @param fmt The video format with lighting info
 * @return CFDataRef or NULL
 */
CFDataRef cvpx_create_content_light_level_data(const video_format_t *fmt);

/**
 * @brief Attach HDR mastering and lighting metadata to a CVPixelBuffer
 * 
 * @param cvpx The image buffer
 * @param fmt The video format
 */
void cvpx_attach_hdr_metadata(CVPixelBufferRef cvpx, const video_format_t *fmt);

/**
 * @brief Extract color properties and HDR metadata from a CVPixelBuffer into video_format_t
 * 
 * @param cvpx The source image buffer
 * @param fmt The destination video format
 */
void cvpx_extract_color_properties(CVPixelBufferRef cvpx, video_format_t *fmt);
